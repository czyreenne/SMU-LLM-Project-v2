...
def save_individual_hypothetical_results(hypo_results: Dict, hypo_filename: str, hypo_original_name: str, shared_results_dir: str) -> None:
    """
    Save results for a single hypothetical across all models
    """
    ...
    try:
        ...
        # Create markdown directory for this hypothetical
        hypo_md_dir = os.path.join(shared_results_dir, hypo_filename)
        os.makedirs(hypo_md_dir, exist_ok=True)
        ...
        for model_name, model_results in hypo_results.items():
            ...
            # Create temporary JSON file for this model's results
            temp_json = os.path.join(hypo_md_dir, f"temp_{model_name}.json")
            ...
            try:
                ...
                md_file = os.path.join(hypo_md_dir, f"{safe_model_name}.md")
                convert_to_md(temp_json, md_file)
                print(f"Markdown saved: {md_file}")
            except Exception as md_error:
                print(f"Warning: Failed to convert {model_name} to markdown: {str(md_error)}")
            finally:
...
def run_single_model_for_hypothetical(model: str, analysis_text: str, api_keys: dict, results_dict):
    ...
    """
    try:
        print(f"[{model}] Starting analysis...")
        ...
        for agent_name, agent in workflow.agents.items():
            print(f"[{model}] Performing analysis using {agent_name}...")
            agent_results = agent.perform_full_structured_analysis(question=analysis_text)
            analysis_results["agent_outputs"][agent_name] = agent_results

        # Synthesize reviews
        print(f"[{model}] Synthesizing perspectives...")
        internal_review = analysis_results["agent_outputs"]["internal"].get("review", "")
        external_review = analysis_results["agent_outputs"]["external"].get("review", "")
